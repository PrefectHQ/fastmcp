"""Tests for the in-process ASGI bridge and `run_server_in_memory`."""
...
class TestRunServerInMemory:
    ...
    async def test_client_round_trip(
        ...
        async with run_server_in_memory(build_server(), transport=transport) as server:
    ...
    async def test_default_path_matches_transport(self):
        async with run_server_in_memory(build_server(), transport="sse") as server:
            assert server.url.endswith("/sse")
        async with run_server_in_memory(build_server(), transport="http") as server:
            assert server.url.endswith("/mcp")

    async def test_custom_path_is_used(self):
        async with run_server_in_memory(build_server(), path="/custom") as server:
    ...
    async def test_server_initiated_request_mid_stream(self):
        ...
        async with run_server_in_memory(build_server()) as server:
            async with Client(
                server.transport(), elicitation_handler=elicitation_handler
            ) as client:
    ...
    async def test_auth_middleware_runs(self):
        ...
        async with run_server_in_memory(server) as in_memory_server:
            async with in_memory_server.http_client() as http:
                unauthenticated = await http.post(
                    in_memory_server.url,
