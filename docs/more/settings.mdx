...
| `FASTMCP_HTTP_SESSION_IDLE_TIMEOUT` | `float \| "auto" \| null` | `"auto"` | Seconds a stateful Streamable HTTP session may remain idle before it is terminated. `"auto"` uses the MCP SDK's default: 1800 seconds (30 minutes) in SDK 2.2, no timeout in earlier versions. Set a positive number to override it, or set `fastmcp.settings.http_session_idle_timeout = None` in Python to disable it. |
| `FASTMCP_DEBUG` | `bool` | `false` | Enable debug mode. |

MCP SDK 2.2 also limits concurrent stateful HTTP sessions to 10,000. Keeping
the default idle timeout lets abandoned sessions expire and frees capacity for
new connections. Disabling it means clients must explicitly close their sessions
to release that capacity. Stateless HTTP does not use the session pool.
