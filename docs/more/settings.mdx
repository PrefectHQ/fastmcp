...
Skipping the blocklist is only safe if the request is *actually* routed through the proxy, so FastMCP verifies that before proceeding. If you set `FASTMCP_SSRF_TRUST_PROXY=true` but no `HTTPS_PROXY` / `ALL_PROXY` is present in the server process's environment (an `HTTP_PROXY` alone never routes these HTTPS-only fetches), or `NO_PROXY` excludes the target host, the request would otherwise go out **direct with the IP blocklist disabled** — no SSRF protection at all. Rather than send it, FastMCP refuses the fetch and raises `SSRFError` with an actionable message. The contract is crisp: proxy-trust mode delegates SSRF protection to the proxy, and with no proxy for a given target the fetch cannot proceed. Enable this setting only together with an active proxy that routes your auth endpoints.
</Warning>
