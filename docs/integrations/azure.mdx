...
By default, `AzureProvider` sends `prompt=select_account` to Azure, so Entra shows an account picker rather than reusing whichever session the browser already holds without asking. Users signed in to several Microsoft accounts, or holding a stale session, can then pick the right one.

Pass `extra_authorize_params` to send different parameters to Azure's authorization endpoint:
...
auth = AzureProvider(
    ...
    extra_authorize_params={"domain_hint": "contoso.com"},
)

mcp = FastMCP("My App", auth=auth)
```

A dict you pass replaces the default rather than merging with it. The example above sends `domain_hint` and no `prompt`. To keep the account picker alongside another parameter, name both:

```python
extra_authorize_params={"prompt": "select_account", "domain_hint": "contoso.com"}
```

Passing `{}` sends no extra parameters at all, which leaves Azure to its own default of silent single sign-on.
...
<Note>
B2C user flows accept `prompt=login` and reject `prompt=select_account`. `from_b2c()` therefore sends no `prompt` at all, rather than the account-picker default described under [Account Selection](#account-selection). Pass `extra_authorize_params={"prompt": "login"}` to make B2C users re-enter their credentials.
</Note>
