...
The client-only package uses the `fastmcp_client` import namespace:
...
Use `fastmcp-client` when your code connects to MCP servers but does not define or run FastMCP servers itself. For example, framework authors can depend on `fastmcp-client` to provide MCP connectivity without requiring users to install the full FastMCP server stack.
...
```bash
pip install "fastmcp-client[openai]"
pip install "fastmcp-client[anthropic]"
pip install "fastmcp-client[gemini]"
```
...
The `fastmcp-client` package is intentionally narrower: it is for client-only consumers who want FastMCP's MCP client behavior without depending on the full framework.
