...
For Auth0 [Auth for MCP](/integrations/auth0#auth-for-mcp-dcr), CIMD registration and client approval happen in the Auth0 tenant dashboard — not through FastMCP's OAuth proxy CIMD flow.
