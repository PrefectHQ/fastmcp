...
By default, providing a `sampling_handler` advertises a basic sampling capability (without tool execution) to the server. If your handler supports executing tools during sampling, you must explicitly advertise the tool capability:
...
To advertise sampling without tool support:
