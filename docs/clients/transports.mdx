...
<Warning>
STDIO servers inherit only a small allowlist of environment variables — `HOME`, `LOGNAME`, `PATH`, `SHELL`, `TERM`, and `USER`. Anything else in your shell, including API keys and other credentials, does not reach the server unless you pass it through `env` explicitly.
</Warning>
