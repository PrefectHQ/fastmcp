...
<Note>
STDIO servers receive the MCP SDK's default subprocess environment. Pass `env` when you need to add or override configuration values for the launched server.
</Note>
...
When an STDIO server needs configuration beyond the default subprocess environment, pass the variables explicitly.
