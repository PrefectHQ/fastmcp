...
The client uses context managers for connection management. When you enter the context, the client establishes a connection and negotiates the protocol era with the server. Both legacy initialization and modern discovery provide capabilities, server metadata, and instructions through the same client properties.
