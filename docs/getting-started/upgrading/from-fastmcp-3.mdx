...
The `mount(as_proxy=...)` flag was already meaningless — a mount always invokes the child's lifespan and middleware — so proxying is now explicit: wrap the child in `create_proxy()` before mounting it.

`import_server` → `mount` is the one row here that is not a mechanical swap, because the two never had the same semantics. `import_server` took a **one-time static snapshot** — it copied the child's tools, resources, and prompts at call time, with no live link, and did not run the child's lifespan or middleware. `mount` is a **live composition** — it holds a live link to the child and runs the child's lifespan and middleware. After switching, later changes to the child become visible through the parent, and the child's lifespan and middleware now execute on every request. If you depended on the frozen-copy behavior (a stable snapshot, no child lifecycle), there is no drop-in replacement: register the child's components on the parent directly instead of composing the two servers.
...
- **Tool `exclude_args=`** is removed. Hide a parameter from the tool schema with dependency injection — declare it with `Depends()` (from `fastmcp.dependencies`) so the caller never sees it.
...
- **`StreamableHttpTransport(sse_read_timeout=...)`** is removed — it was a no-op under the SDK v2 client. Configure timeouts through `read_timeout_seconds` in `session_kwargs`, or on the httpx2 client via `httpx_client_factory`. (`SSETransport` still accepts `sse_read_timeout`.)
...
**Resource-not-found now returns `-32602`.** The wire error code for a missing resource from the core `resources/read` handler changed from `-32002` to `-32602` (`INVALID_PARAMS`, per SEP-2164). The human-readable message ("Resource not found: ...") is unchanged, so this only affects clients that matched on the numeric code — update those to expect `-32602`.
...
1. **Bump your environment.** Raise any pin below `pydantic>=2.12`; upgrade FastAPI if your resolver complains about Starlette `<1.0`.
2. **Fix imports that moved out.** Replace `from mcp.types import X` with `from fastmcp.types import X` (or `import mcp_types`), and update any import from the [removed modules](#moved-imports) (`fastmcp.server.proxy`, `fastmcp.server.openapi`, `fastmcp.server.apps`, the `fastmcp.tools.tool` / `resources.resource` / `prompts.prompt` component shims).
