...
**Custom client HTTP now uses httpx2.** The SDK v2 replaces `httpx` with [httpx2](https://pypi.org/project/httpx2/), a next-generation httpx fork, for the client transport stack. If you pass a custom client or factory into a FastMCP client transport — `StreamableHttpTransport(httpx_client_factory=...)`, `SSETransport(httpx_client_factory=...)`, `OAuth(httpx_client_factory=...)`, or a custom `httpx.Auth` as `Client(auth=...)` — those objects must now be httpx2. httpx2 is a drop-in fork with the same public API, so the change is an import swap:
...
Server-side HTTP that FastMCP owns — the `httpx.AsyncClient` you pass to `FastMCP.from_openapi(client=...)`, custom auth providers' upstream calls, and HTTP made inside your own tools — stays on httpx and needs no change.

Two runtime behaviors shift with httpx2 on the client path. TLS verification uses the operating system's trust store (via `truststore`, honoring `SSL_CERT_FILE`/`SSL_CERT_DIR`) instead of the bundled certifi CA set, so corporate-CA or certifi-pinned setups may verify differently. And the client HTTP loggers are renamed from `httpx`/`httpcore.*` to `httpx2`/`httpcore2.*` — update any logging filters that select the client HTTP stack by logger name.
