...
See [Troubleshooting](#troubleshooting) if that doesn't resolve it.
...
If `import fastmcp` raises `ModuleNotFoundError`, or `from fastmcp import FastMCP` raises `ImportError`, immediately after a `pip` upgrade, your install is in a half-removed state. FastMCP 3.3 moved the importable code from the `fastmcp` distribution into `fastmcp-slim`. When pip upgrades from 3.2 or earlier in a single command, it can install the new files and then remove them while uninstalling the old `fastmcp` distribution, whose file manifest still lists those paths.

Reinstall to restore both the importable package and the `fastmcp` CLI:
...
`uv` uninstalls before it installs, so `uv pip install --upgrade fastmcp` and `uv add fastmcp` are unaffected.
