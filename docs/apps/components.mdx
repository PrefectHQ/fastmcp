...
<Note>
All components below are imported from `prefab_ui.components` unless otherwise noted. Charts live in `prefab_ui.components.charts` but are re-exported from `prefab_ui.components` for convenience.
</Note>
...
`DataTableColumn` takes `key` (the dict key), `header` (display name), and `sortable` (per-column override).
...
|-------------|------|-------------|
| `default_value` | `str` | Label of the initially active tab |
