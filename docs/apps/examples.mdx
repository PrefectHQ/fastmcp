...
Separates model-visible media discovery from an interactive carousel. `Play` and `Save` call app-only backend tools directly, while `More like this` sends a follow-up request to the conversation. The normalized candidate shape keeps the UI independent of YouTube, Internet Archive, live cameras, or future source adapters.

```bash
fastmcp dev apps examples/apps/media_picker/media_picker_server.py
```
