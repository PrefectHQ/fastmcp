...
That means `app=True` is opt-in, while type inference is conservative: if the return type is a Prefab type and you haven't set `app` explicitly, FastMCP auto-wires the metadata as if you'd written `app=True`.
