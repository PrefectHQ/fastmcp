...
The placeholder name `{session_id}` is special. When a topic is declared with `{session_id}`, FastMCP automatically enforces that each subscriber can only subscribe to their own session's slot:

- A subscriber **must** substitute their own session UUID in the `{session_id}` position.
- Wildcards (`+`, `#`) in the `{session_id}` position are rejected.
...
```python
# Session-scoped: only the owning session can subscribe
mcp.declare_event("app/sessions/{session_id}/messages")

# Public: any session can subscribe
mcp.declare_event("app/server/status")

# Public with descriptive placeholder: {project} is not magic
mcp.declare_event("app/builds/{project}/status")
```

### The `authorize` Callback

For custom authorization beyond `{session_id}`, use the `authorize` parameter on `declare_event`:
...
- If `authorize` is set, it **overrides** the default `{session_id}` check entirely. The callback is fully responsible for authorization.
...
This is the UUID that the `{session_id}` convention enforces against.
...
- Useful for defense-in-depth routing alongside `{session_id}` scoping.
