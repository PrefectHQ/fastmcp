...
<VersionBadge version="2.13.0" />
  ...
  **Default behavior (`None`):**
  The key is deterministically derived from `upstream_client_secret` using HKDF, on every platform. Because the derivation is deterministic, the same key is produced across restarts as long as `upstream_client_secret` doesn't change, so tokens remain valid without any extra configuration. This convenience makes it **only** suitable for development and local testing.
