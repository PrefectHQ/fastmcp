...
<Note>
For Auth0 [Auth for MCP](/integrations/auth0#auth-for-mcp-dcr) with tenant-level DCR, use [`Auth0MCPProvider`](/integrations/auth0) with [`RemoteAuthProvider`](/servers/auth/remote-oauth) instead of `Auth0Provider`.
</Note>
