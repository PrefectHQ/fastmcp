...
| ------------------ | --------------------- |
| `"native"` (default) | Loopback `http` (`http://127.0.0.1`, `http://localhost`, `http://[::1]`, any port), `https`, and OS-registered private-use/custom schemes (e.g. `com.example.app:/callback`, `myapp://callback`) |
| `"web"` | `https` on a non-loopback host only |

Both types always reject unsafe browser schemes (`javascript:`, `data:`, `file:`, `vbscript:`), and native clients additionally reject non-loopback plain `http` and network-transport schemes such as `ftp:` or `ws:`.
