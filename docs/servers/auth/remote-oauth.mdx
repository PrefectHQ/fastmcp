...
## Auth0 Auth for MCP Integration

Auth0's [Auth for MCP](https://auth0.com/ai/docs/mcp/intro/overview) uses the same resource-server pattern. The `Auth0MCPProvider` discovers your tenant via OIDC, validates JWTs (including Auth0 `permissions` claims), and forwards authorization server metadata.
...
Enable Auth0's Resource Parameter Compatibility Profile and set your API identifier to the resource URL the server logs at startup.

→ **Complete Auth0 tutorial**: [Auth0 Integration Guide](/integrations/auth0#auth-for-mcp-dcr)
