...
<Tip>
To ensure a component is never exposed, disable it at the server level with `mcp.disable(...)`. Filter by `keys` when you need an exact component, and include the component key's trailing version delimiter (for example, `tool:admin_tool@` for an unversioned tool).
</Tip>
