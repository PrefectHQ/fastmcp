...
The handler only runs for references the caller can list. A prompt or template hidden by `disable()`, component auth, or middleware such as `AuthMiddleware` gets an empty completion, as an unknown reference would, so suggestions never reveal what the listing withholds.
