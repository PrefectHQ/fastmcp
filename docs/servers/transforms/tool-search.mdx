...
Pinned tools appear directly in `list_tools` so the LLM can call them without searching. They're excluded from search results to avoid duplication, and the `call_tool` proxy refuses them: a pinned tool is called by its own name, so a host that approves tools one at a time sees it under its own annotations.
...
The proxy rejects attempts to call the synthetic tools themselves. `call_tool(name="call_tool")` raises an error rather than recursing. It also rejects pinned tools, so the proxy reaches exactly the tools search can return.

The proxy's [annotations](/servers/tools#mcp-annotations) summarize those tools, taking the least permissive value of each hint. If every searchable tool is marked `readOnlyHint: true`, so is the proxy, and a host that auto-approves read-only tools can approve it too. One unannotated tool is enough to make the proxy a destructive write, because unset hints take the MCP defaults.
...
# delete_record is listed with its own annotations;
# call_tool is read-only because it can reach only the two read tools
mcp.add_transform(BM25SearchTransform(always_visible=["delete_record"]))
```

The proxy's annotations and its pinned-tool check are computed from the tools the search transform sees. Add the search transform after any transform that renames or adds tools, such as `Namespace`, `ToolTransform`, or `PromptsAsTools`; a transform added later changes names and tools the proxy can't account for.

Those tools include ones that are disabled or restricted by auth, so a disabled write tool still makes the proxy a write. The hints can be stricter than what a caller can reach, never looser. To keep a disabled tool out of the proxy's hints, pin it with `always_visible`; a disabled tool is still left out of the listing.
