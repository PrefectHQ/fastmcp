...
| `shortlist` | 8 | Candidates each wide-pass request carries forward; with `close_read=True`, it must be smaller than `chunk_size`. The close read sees at most three times this or 255 tools, whichever is smaller. |
