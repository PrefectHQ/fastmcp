...
- **`ctx.session_id -> str`**: Get the identifier for the current MCP connection. It remains stable across calls on stateful legacy, handshake-era sessions; modern sessionless connections and stateless HTTP transports receive a fresh identifier for each request. Use [Session State](/servers/sessions) for data that must persist across requests. Raises `RuntimeError` when no connection is available.

When you control the client and specifically need legacy connection state, use `Client(server, mode="legacy")` with a stateful transport. Prefer [Session State](/servers/sessions) for application data because it works across protocol eras and transport modes.
