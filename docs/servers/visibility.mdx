...
<Warning>
Keys are matched by exact string equality. A key that omits the trailing `@` — `tool:delete_everything` rather than `tool:delete_everything@` — matches nothing and the call silently has no effect. Prefer `names` unless you specifically need version-level precision, and read a key off `component.key` rather than assembling it by hand.
</Warning>

### Combining Filters

You can specify several criteria in a single call. The filters combine additively.

```python
# Disable a specific tool AND all dangerous-tagged components
