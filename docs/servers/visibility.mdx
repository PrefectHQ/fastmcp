...
Every component has a unique key in the format `{type}:{identifier}@{version}`. The `@` delimiter is always present; unversioned components use an empty version suffix such as `tool:get_status@`.
