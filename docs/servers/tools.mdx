...
By default, exceptions are converted into an MCP error response to be sent back to the client LLM. This helps the LLM understand failures and react appropriately. Validation warnings identify the tool without logging argument values, validation locations, or validator messages, which may contain private client data. This does not change the validation details returned to the caller or redact explicit debug logging or custom middleware logs.
