...
<ParamField body="run_in_thread" type="bool" default="True">
  Whether sync tool functions run in a worker thread. When `True` (default), sync functions are dispatched to a thread pool so they don't block the event loop. Set to `False` to run the function inline on the event loop thread — useful for libraries with thread affinity like Windows COM (`pywin32`, `uiautomation`, `comtypes`), `tkinter`, or certain GPU/driver bindings. Has no effect on async functions. See [Thread affinity](#thread-affinity) for details.
</ParamField>
</Card>
