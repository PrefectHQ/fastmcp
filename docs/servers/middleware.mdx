...
- **Completion** (`completion/complete`) runs the parent's middleware, not the mounted server's, even when the reference names a mounted server's prompt or template
