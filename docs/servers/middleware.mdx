...
Called when a client connects and initializes the session. The initialization result flows back through middleware, so you can inspect or replace it by returning a different `InitializeResult`.
...
**Returns:** `InitializeResult | None` — Return the result from `call_next()` unless you intentionally replace it.
