...
Dispatch begins at the SDK seam, the single point every inbound message passes through. As a result, `on_message`, `on_request`, and `on_notification` observe **every** message a client sends, including the ones that never reach a tool, resource, or prompt handler:
...
#### Guard-tool suspensions

A guard tool suspends by returning an `InputRequiredResult` to ask the client for input (see [Multi-round-trip tools](/servers/tools)). Internally this travels as a `BaseException` control signal, mirroring `asyncio.CancelledError`: a middleware's `call_next` **raises** rather than returning a value, so a hook that records or transforms results neither observes the suspension as a result nor accidentally swallows it. A middleware that legitimately uses a broad `except Exception` is unaffected—the signal is a `BaseException`, so it passes straight through. The `InputRequiredResult` is produced at the wire boundary and sent to the client; it is not handed back up the FastMCP chain as a component result. To act on a suspension deliberately, catch it by intent rather than with a broad handler.
