...
Called when a client connects and initializes the session. Middleware can reject the client before `call_next()` raises an error response, or inspect the `InitializeResult` after `call_next()` returns.
...
async def on_initialize(self, context: MiddlewareContext, call_next):
    client_info = getattr(context.message.params, "clientInfo", None)
...
**Returns:** `InitializeResult | None` — Return the result from `call_next()`. The initialize response has already been sent by the low-level handler, so returning a different `InitializeResult` after `call_next()` won't replace what the client receives.
