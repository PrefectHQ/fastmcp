...
`ToolInjectionMiddleware` dynamically injects tools during request processing.
...
<Note>
`PromptToolMiddleware` and `ResourceToolMiddleware` are deprecated. Use the [ResourcesAsTools and PromptsAsTools transforms](/servers/transforms) instead, which provide better support for resource templates and other features.
</Note>
