...
from fastmcp import FastMCP
from fastmcp.resources import DirectoryResource, FileResource, HttpResource, TextResource
...
# 4. Serving a web page fetched by the server
page_resource = HttpResource(
    uri="docs://service-public/f23533",
    url="https://entreprendre.service-public.gouv.fr/vosdroits/F23533",
    name="Service Public F23533",
    mime_type="text/html",
)
mcp.add_resource(page_resource)
```

`HttpResource` has two addresses. `uri` is the identity clients see in the resource list, and `url` is what the server fetches when a client reads it. Keep them different: the MCP specification reserves `https://` URIs for content the client can fetch on its own, so advertising the fetched URL as the `uri` tells clients they don't need your server for it. A custom scheme like `docs://` makes it clear the content comes through the server.
...
-   `HttpResource`: Fetches content from an HTTP(S) URL on the server side when a client reads it (requires `httpx2`).
