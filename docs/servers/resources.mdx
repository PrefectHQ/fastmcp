...
-   **JSON-native values** (`dict`, `list`, `tuple`, numbers, booleans, or `None`): Serialized to JSON text. The declared MIME type differs by component: a concrete resource keeps its default `text/plain`, while a resource template reports `application/json`. Set `mime_type="application/json"` explicitly whenever clients rely on the content type to parse the payload, so both paths agree.
