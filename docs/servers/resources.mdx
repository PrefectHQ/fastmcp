...
-   **JSON-native values** (`dict`, `list`, `tuple`, numbers, booleans, or `None`): Serialized to JSON text with `mime_type="application/json"`.
-   **`ResourceContent`**: A single content item with explicit MIME type and item metadata.
...
<Note>
For custom classes that are not JSON-native, return a `ResourceResult` or wrap values in `ResourceContent` so serialization and MIME types are explicit.
</Note>
