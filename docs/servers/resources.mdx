...
-   **JSON-native values** (`dict`, `list`, `tuple`, numbers, booleans, or `None`): Serialized to JSON text. The declared MIME type stays `text/plain` unless you set `mime_type="application/json"` on the decorator, so set it explicitly when clients rely on the content type to parse the payload.
