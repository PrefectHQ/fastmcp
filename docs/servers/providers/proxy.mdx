...
The proxy sits between your client and the source server: the client sends an MCP request to the proxy, the proxy forwards it over whichever transport the backend speaks, and the response comes back the same way.
