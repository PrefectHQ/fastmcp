...
| **Multi-round tools (SEP-2322)** | A tool suspends by returning an `InputRequiredResult` and re-runs per round, reading the client's answers off `ctx.input_responses` / `ctx.request_state` (the [guard pattern](/servers/multi-round-tools)). The framework seals `request_state` on the wire and unseals it before the tool runs; a shared-key `request_state_security` policy carries state across replicas. On handshake-era connections a suspend produces a clear era error. |
