...
While in-memory testing covers most unit testing needs, you'll occasionally need to test actual network transports like HTTP or SSE. FastMCP provides three approaches, in order of preference: in-memory HTTP servers (fastest), in-process async servers on a real port, and separate subprocess servers (for special cases).

#### In-Memory HTTP Testing (Fastest)

<VersionBadge version="3.5.0" />

`run_server_in_memory` runs your server's real Starlette app without a socket. Requests are dispatched straight into the ASGI application on the current event loop, so there is no port to bind and no uvicorn to start — but the full HTTP stack still runs. Middleware, authentication, session management and SSE streaming all behave exactly as they do in production.

It yields an `InMemoryServer` rather than a URL string, because nothing is listening on the network: a plain `httpx.AsyncClient()` cannot reach the server, so the object hands you clients that can.
...
async def http_server():
    """Run the server's HTTP app in-process."""
    server = create_test_server()
    async with run_server_in_memory(server) as in_memory_server:
        yield in_memory_server

async def test_http_transport(http_server: InMemoryServer):
    """Test actual HTTP transport behavior, with no socket involved."""
    async with Client(http_server.transport()) as client:
        greeting = await client.call_tool("greet", {"name": "World"})
        assert greeting.data == "Hello, World!"
```

`transport()` builds a client transport already wired to the in-process app, and accepts the same arguments as `StreamableHttpTransport` or `SSETransport` — including `headers` and `auth`. Pass `transport="sse"` to `run_server_in_memory` to exercise the SSE app instead.

For assertions about raw HTTP — status codes, response headers, metadata endpoints — use `http_client()`, which returns an `httpx.AsyncClient` bound to the same app:

```python
async def test_unauthenticated_request_is_rejected(http_server: InMemoryServer):
    async with http_server.http_client() as http:
        response = await http.post(http_server.url, json={"jsonrpc": "2.0", "id": 1})
        assert response.status_code == 401
```

Prefer this approach unless the behavior under test is genuinely about the network — real TCP, TLS, or a separate process — in which case use one of the approaches below.

#### In-Process Network Testing on a Real Port

<VersionBadge version="2.13.0" />

When a test's subject is real network behavior, use `run_server_async` as an async context manager. This runs a real uvicorn server as a task in the same process, bound to a real port, and yields its URL:
