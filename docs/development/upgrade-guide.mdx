...
BEHAVIOR CHANGES (won't crash but may surprise):

- Decorators (@mcp.tool, @mcp.resource, @mcp.prompt) now return the original function, not a component object. Code that treats the result as FunctionTool will break. Set FASTMCP_DECORATOR_MODE=object for v2 compat.
...
- `host`, `port`, `log_level`, `debug`, `sse_path`, `message_path`, `streamable_http_path`, `json_response`, `stateless_http` — pass to `run()`, `run_http_async()`, or `http_app()`, or set via environment variables (e.g. `FASTMCP_HOST`)
...
If you were passing a `DiskStore` explicitly, you can either [switch to `FileTreeStore`](/servers/storage-backends) (recommended) or keep using `DiskStore` by adding `pip install 'py-key-value-aio[disk]'` to your dependencies, which re-introduces the vulnerable `diskcache` package.
