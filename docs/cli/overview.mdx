...
| [`install`](/cli/installing) | Install a server into Claude Code, Claude Desktop, Cursor, Gemini CLI, or Goose |
| [`inspect`](/cli/inspecting) | Print a server's tools, resources, and prompts as a summary or JSON report |
| [`list`](/cli/querying) | List a server's tools (and optionally resources and prompts) |
| [`call`](/cli/querying#calling-tools) | Call a single tool with arguments |
| [`discover`](/cli/querying#discovering-configured-servers) | Find MCP servers configured in your editors and tools |
...
Run [`fastmcp discover`](/cli/querying#discovering-configured-servers) to see what's available on your machine.

### Factory Functions

<VersionBadge version="2.11.2" />

Several commands support factory functions — functions that create and return a server instance. This is useful when your server needs setup that would normally live in `if __name__ == "__main__"` (which `fastmcp run` ignores). Both sync and async factories work:
...
FastMCP detects that you've pointed at a callable rather than an instance and invokes it automatically.
