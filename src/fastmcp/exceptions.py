...
class FastMCPError(Exception):
    ...
    def from_generic_exception(
        ...
        if isinstance(exc, httpx.HTTPStatusError):
            ...
            if status_code == 401:
                new_exc = MCPAuthorizationError(
                    f"Authorization failed: {error_msg}",
                    subtype="invalid_token",
                )
            elif status_code == 404:
                new_exc = NotFoundError(f"Not found: {error_msg}", resource_type="generic")
            elif 400 <= status_code < 500:
                new_exc = MCPTransportError(
                    f"HTTP error {status_code}: {error_msg}",
                    data={"status_code": status_code},
                )
            else:
                new_exc = FastMCPError(f"Server error: {error_msg}")
        ...
        elif exc_type is ConnectionError or isinstance(exc, httpx.ConnectError):
