...
class FastMCPError(Exception):
    ...
    def from_generic_exception(
        ...
        if isinstance(exc, McpError):
            return FastMCPError.from_mcp_error(exc)
        ...
        if isinstance(exc, NotFoundError):
            if method and method.startswith("resources/"):
                return NotFoundError(f"Resource not found: {exc_str}", resource_type="resource")
            return NotFoundError(f"Not found: {exc_str}", resource_type="generic")

        if isinstance(exc, ToolError):
            if exc_type in (ValueError, TypeError):
                return InvalidParamsError(f"Invalid params: {exc_str}")
        ...
        if exc_type in (ValueError, TypeError):
            ...
            return MCPAuthorizationError(
                f"Permission denied: {exc_str}",
                subtype="insufficient_scope",
            )

        elif exc_type in (TimeoutError,):
            return MCPTimeoutError(f"Request timeout: {exc_str}")

        elif exc_type is ConnectionError:
            return MCPConnectionError(f"Connection error: {exc_str}")
        ...
        elif isinstance(exc, FastMCPError):
            return exc

        else:
            return InternalError(f"Internal error: {exc_str}")
